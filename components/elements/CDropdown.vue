<template>
  <div :class="['container', style]">
    <div :class="required && 'field-label-require'" class="field-label">{{ label }}</div>
    <div
      class="aselect"
      tabindex="100"
      ...
      <div class="selector" :class="paddingStyle" @click="toggle()">
        <div class="label">
          <span :class="!props.modelValue && 'placeholder-text'">{{
            find(props.options, ({ value }) => props.modelValue === value)?.['name'] || props.placeholder
          }}</span>
        </div>
        <div v-if="!!icon" :class="'icon-' + props.iconName" class="icon-left-style"></div>
    ...
    <div class="error-container">
      <span v-if="!!props.errorMessage" class="error">{{ props.errorMessage }}</span>
    </div>
  </div>
</template>
...
const backgroundIconStyle = computed(() => {
  if (props.errorMessage) {
    return 'urgen'
  } else {
    return 'primary'
...
.aselect {
  ...
  .selector {
    ...
    .icon-chevron-down {
      ...
      top: 11px;
      flex-shrink: 0;
      @include icon('chev-down', 24px, $pale);
    }
  }
  ul {
    left: 0;
